...
exports.signup = async (req, res) => {
  // Finding user from the database from req.body
  ...
  // Once we create new user we await the user to be saved
  await user.save();
  res.status(200).json({ message: 'Signup successfull! Please login' });
};

exports.signin = (req, res) => {
  // Find the user based on email to check if the user exist in the database
  ...
  User.findOne({ email }, (err, user) => {
    ...
    // If user exist, make sure the email and password match
    ...
    // We generate a token with user id and secret
...
exports.requireSignin = expressJwt({
  // If the token is valid, exress jwt appends the verified users id
